...
public class yoloDetector {
    private Net net;
    private Rect qrBox; // Lưu tọa độ QR Code được phát hiện
    ...
    public Mat detectQR(Mat frame) {
        ...
        for (Mat output : outputs) {
            for (int i = 0; i < output.rows(); i++) {
                ...
                if (confidence > 0.5) { // Ngưỡng xác suất
                    int x = (int) (data[0] * frame.cols());
                    int y = (int) (data[1] * frame.rows());
                    int width = (int) (data[2] * frame.cols());
                    int height = (int) (data[3] * frame.rows());

                    qrBox = new Rect(x, y, width, height); // Lưu bounding box QR Code
    ...
    public Rect getQRBox() {
        return qrBox; // Trả về tọa độ QR Code để cắt ảnh
    }
}
